...
module.exports = async function req(token, method, endpoint, data=null, body=null) {
  ...
  if (typeof(Http[method]) === 'function') return Http[method](token, url, data, body);
  else return false;
}
...
class Http {
  ...
  static async post(token, url, data, body) {
    var response;
    var headers = {
      Authorization: 'Bearer ' + token
    };
    ...
    try {
      response = await axios.post(url, { headers: headers });
    } catch(err) {
      if(!err.response) return err;
